...
-- =============================================
CREATE PROCEDURE [dbo].[sp_OPS_subqry_UpdateRigMoveRentals]
...
BEGIN
	...
	FROM OPS_RigMoveRentals
	WHERE 
	--FK_RouteSurveyId=@routesurveyid AND 
	StatusFlag=1 AND FK_RentalsId=@fbid
    
	IF(@flag=1 AND @fid IS null) -- Adding FB to the fleet
		BEGIN
        --insert new flatbed into rig move fleet
			INSERT INTO OPS_RigMoveRentals
			(FK_RouteSurveyId,FK_RentalsId,DateofCreation,DateofMobilization,DateOfRelease,StartingOdometer,EndingOdometer,Remarks,Statusflag)
			VALUES
    ...
    BEGIN
		...
		SET DateofMobilization=@dateofmob,
			...
			Statusflag=0
		WHERE
        FK_RouteSurveyId=@routesurveyid AND FK_RentalsId=@fbid
